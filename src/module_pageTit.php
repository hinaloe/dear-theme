...
$thumb = is_singular() && has_post_thumbnail() ? wp_get_attachment_url( get_post_thumbnail_id() ) : null;
...
if ( ! $thumb ) {
	...
	// single and having thumb post
	$pageTitHtml_before = '<div class="section page-header cover" style="background-image: url(' . $thumb . ')"><div class="container"><div class="row"><div class="col-md-12">'. PHP_EOL;
	$pageTitHtml_before .= '<' . $pageTitTag . ' class="page-header_pageTitle">' . PHP_EOL;
	$pageTitHtml_after = '</' . $pageTitTag . '>' . PHP_EOL;
	$pageTitHtml_after .= '</div></div></div></div><!-- [ /.page-header ] -->' . PHP_EOL;
